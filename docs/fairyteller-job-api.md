...
- A one-off operational resend uses the same follow-up template and is available only from the server shell: `node /opt/fairyteller-api/fairyteller-api.mjs --send-follow-up=<email>`. It refuses unsubscribed recipients, includes up to three ready unpaid stories, and starts the normal 24-hour follow-up cooldown after a successful send.
