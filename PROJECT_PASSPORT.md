...
- Current first working renderer: Node.js `pdf-lib` generator with embedded DejaVu fonts, exact page sizes, Job API file outputs, master PDF backgrounds from `server/templates/MasterTemplate_book.pdf` and `server/templates/MasterTemplate_cover_romantic.pdf`, and a versioned layout contract at `server/render-layouts/fairyteller-template-v1.json`.
