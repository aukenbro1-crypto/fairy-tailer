...
- Dynamic content should replace template placeholders only: cover title, cover subtitle/summary, cover art, chapter titles, chapter illustrations, five text pages per chapter, and the final QR/coauthor page. Do not add extra colored rectangles, duplicate `by FairyTeller`, or extra image-only chapter pages outside the PPTX page plan.
...
- The PDF renderer must remain a deterministic layout/preflight layer. It should not rewrite or silently trim story content. If a chapter has anything other than the expected five print-ready text blocks, or if a block does not fit inside the declared template box, render preflight must fail so the upstream editor step can repair the text.
- The n8n text chain should preserve the Make-era author/editor split: the author creates the story, then an editor/prepress step reshapes it into `printReadyChapters` with exactly five blocks per chapter and text sized for the fixed PDF template.
...
- Replaced the PDF-template approximation with a PPTX-extracted renderer contract (`fairyteller-pptx-v2`). The cover now uses the clean PPTX background, places cover art only inside the white frame, draws title/subtitle/summary without extra colored backing, and keeps the existing `by FairyTeller` mark instead of duplicating it. Interior pages now follow the 40-slide book plan: title page, subtitle/dedication/TOC, each chapter opener with title plus illustration, five text pages per chapter, and the QR/coauthor final page. Deployed renderer/assets/fonts/layout to `/opt/fairyteller-render`, re-rendered `ft_1779550092673_8ltt9c`, and verified `book.pdf` remains 41 pages with `render.preflight.noTextTruncation=true`.
