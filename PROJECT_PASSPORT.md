...
Current state as of 2026-05-23: the internal text, full-text, visuals, and render/publish workflows are published and connected to the Job API. The text workflow generates a real first chapter with Gemini and writes `text.json`; a separate full-text workflow generates chapters 2-5 in the background and writes `full-text.json`; the visuals workflow generates a hero reference sheet, then the first chapter illustration, and writes `hero-reference-sheet.png`, `chapter-1.png`, and `visuals.json`; render/publish still uses placeholder contracts.
...
- `fairyteller_text` also starts `fairyteller_full_text` asynchronously after the first chapter payload is restored. This keeps the UX preview path fast while preparing the complete book text in `full-text.json`.
