...
- Added a branded customer completion email template on May 28, 2026 in the Job API. The email now follows the current black/white/peach/yellow Fairyteller site language, uses the title "Ваша книга почти готова", keeps "Открыть книгу" as a quieter secondary CTA, highlights "Оплатить заказ" through `/pay?pdf=...`, includes Telegram/site contact links, plain-text fallback, and `Reply-To` through `FAIRYTELLER_MAIL_REPLY_TO`; it still sends only after real PDF render completion.
